...
class MatplotlibWidget(QtWidgets.QMainWindow):
    def __init__(self, temp_ear_data, temp_time_data):
        super().__init__()
...
class MainWindow(QWidget):
    def __init__(self, time_secs, ear_vals, mar_vals, ear_and_mar_vals, blink_list, yawn_list, temp_ear_data, temp_time_data, ear_threshold, ear_open_value, perclos_vals):
        ...
        #self.MatplotlibSubWidget = MatplotlibWidget(temp_ear_data, temp_time_data)
        #self.VBL.addWidget(self.MatplotlibSubWidget)


    def MatplotlibUpdateSlot(self, img):
        self.FeedLabel.setPixmap(QPixmap.fromImage(QImage(img, img.shape[1], img.shape[0], QImage.Format_RGBA8888)))
                ...
                while (vs.stream.isOpened()):
                    ...
                    if True:
                        ...
                        if results.multi_face_landmarks == None:
                            ...
                            cv2.putText(flippedImage, f"PERCLOS: {str(round(perclos, 4))}", (0, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
